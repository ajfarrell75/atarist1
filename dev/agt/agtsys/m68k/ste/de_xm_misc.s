...
*-------------------------------------------------------*
_BLiT_EntityDebugVisibleXM_68k:
*-------------------------------------------------------*
			.abs
*-------------------------------------------------------*
.savea6:		ds.l	1
...
*-------------------------------------------------------*
	...
	bra		.STE_EntityDebugVisibleXM_RDEBUG_done

*=======================================================*
*	dedicated iterator for [drawtype]=RFILL
*-------------------------------------------------------*
.STE_EntityDebugVisibleXM_RDEBUG_next:
*-------------------------------------------------------*	
*-------------------------------------------------------*
.STE_EntityDebugVisibleXM_RDEBUG_go:
*-------------------------------------------------------*
...
*-------------------------------------------------------*
.STE_EntityDebugVisibleXM_RDEBUG_done:
*-------------------------------------------------------*
	move.l		ent_pnext_x(a0),a0		; next visible entity
	move.w		ent_frx(a0),d1
	cmp.w		.sp_pxymax_(sp),d1
	blt		.STE_EntityDebugVisibleXM_RDEBUG_next
	bra		.STE_EntityDebugVisibleXM_complete
*-------------------------------------------------------*
*	[drawtype] has suddenly changed to RFILL
*-------------------------------------------------------*
.STE_EntityDebugVisibleXM_RDEBUG_setup:
*-------------------------------------------------------*
	jsr		_AGT_BLiT_IMSprInit			; todo: init type depends on platform & spr format
	bra		.STE_EntityDebugVisibleXM_RDEBUG_go

*=======================================================*
.STE_EntityDebugVisibleXM_complete:
*-------------------------------------------------------*
...
;	add.w		dctx_guardy(a4),d2	
	...
	move.l		ent_pnext_x(a2),a0		; pcurr
	cmp.w		ent_frx(a0),d1			; (pcurr->rx < pxmax)
	ble		.term_outer
*-------------------------------------------------------*
.next_outer:
*-------------------------------------------------------*
...
*-------------------------------------------------------*
	move.l		ent_pnext_x(a0),a1		; petest = e.pnext_x;
	cmp.w		ent_frx(a1),d3			; (petest->rx < hmax)
	ble.s		.term_inner
...
*-------------------------------------------------------*
	...
	move.w		ent_fry(a1),d5
	cmp.w		d5,d4				; (petest->ry < vmax)
	ble.s		.no_contact
	add.w		ent_sy(a1),d5			; ((petest->ry + petest->sy) > e.ry)
	cmp.w		ent_fry(a0),d5
	ble.s		.no_contact
...
.no_contact:
*-------------------------------------------------------*
	move.l		ent_pnext_x(a1),a1		; petest = ptest->pnext_x;
	cmp.w		ent_frx(a1),d3
	bgt.s		.next_inner
...
*-------------------------------------------------------*
_BLiT_EntityOccludeVisibleXM_68k:
*-------------------------------------------------------*
			.abs
*-------------------------------------------------------*
.savea6:		ds.l	1
...
*-------------------------------------------------------*
	...
	bra		.STE_EntityOccludeVisibleXM_NONE_done


*=======================================================*
*	dedicated iterator for [drawtype]=EMXSPR
*-------------------------------------------------------*
.STE_EntityOccludeVisibleXM_EMXSPR_next:
*-------------------------------------------------------*
	move.w		ent_drawtype(a0),d0
	cmp.w		.sp_drawtype(sp),d0
	bne		.STE_EntityOccludeVisibleXM_evaluate
*-------------------------------------------------------*
.STE_EntityOccludeVisibleXM_EMXSPR_setup:
.STE_EntityOccludeVisibleXM_EMXSPR_go:
*-------------------------------------------------------*
...
;	add.w		d0,d0
	...
	guard_reject	a2,d1,d3,d0,d6,.STE_EntityOccludeVisibleXM_EMXSPR_no	
	guard_reject	a2,d2,d4,d0,d6,.STE_EntityOccludeVisibleXM_EMXSPR_no	

	move.w		(a0),d0
	beq.s		.STE_EntityOccludeVisibleXM_EMXSPR_no
...
.STE_EntityOccludeVisibleXM_EMXSPR_no:
...
*-------------------------------------------------------*
.STE_EntityOccludeVisibleXM_EMXSPR_done:
*-------------------------------------------------------*
	move.l		ent_pnext_x(a0),a0		; next visible entity
	move.w		ent_frx(a0),d1
	cmp.w		.sp_pxymax_(sp),d1
	blt		.STE_EntityOccludeVisibleXM_EMXSPR_next
	bra		.STE_EntityOccludeVisibleXM_complete
*-------------------------------------------------------*
...
*-------------------------------------------------------*
.STE_EntityOccludeVisibleXM_EMSPR_next:
*-------------------------------------------------------*
	move.w		ent_drawtype(a0),d0
	cmp.w		.sp_drawtype(sp),d0
	bne		.STE_EntityOccludeVisibleXM_evaluate
*-------------------------------------------------------*
.STE_EntityOccludeVisibleXM_EMSPR_setup:
.STE_EntityOccludeVisibleXM_EMSPR_go:
*-------------------------------------------------------*
...
;	add.w		d0,d0
	...
	move.l		.sp_guardwin(sp),a2
	guard_reject	a2,d1,d3,d0,d6,.STE_EntityOccludeVisibleXM_EMSPR_no	
	guard_reject	a2,d2,d4,d0,d6,.STE_EntityOccludeVisibleXM_EMSPR_no	

	move.w		(a0),d0
	beq.s		.STE_EntityOccludeVisibleXM_EMSPR_no
...
.STE_EntityOccludeVisibleXM_EMSPR_no:	
	;

	move.l		.sp_savea0(sp),a0

*-------------------------------------------------------*
.STE_EntityOccludeVisibleXM_EMSPR_done:
*-------------------------------------------------------*
	move.l		ent_pnext_x(a0),a0		; next visible entity
	move.w		ent_frx(a0),d1
	cmp.w		.sp_pxymax_(sp),d1
	blt		.STE_EntityOccludeVisibleXM_EMSPR_next
	bra		.STE_EntityOccludeVisibleXM_complete
*-------------------------------------------------------*
...
*-------------------------------------------------------*
.STE_EntityOccludeVisibleXM_evaluate:
*-------------------------------------------------------*
	move.w		d0,.sp_drawtype(sp)
	add.w		d0,d0
	add.w		d0,d0
	jmp		.types(pc,d0.w)
.types:	bra.w		.STE_EntityOccludeVisibleXM_NONE_setup	; none
	bra.w		.STE_EntityOccludeVisibleXM_NONE_setup	; IMSPR/SPRITE
	bra.w		.STE_EntityOccludeVisibleXM_EMSPR_setup
	bra.w		.STE_EntityOccludeVisibleXM_NONE_setup	; EMSPRQ
	bra.w		.STE_EntityOccludeVisibleXM_EMXSPR_setup
	bra.w		.STE_EntityOccludeVisibleXM_NONE_setup	; EMXSPRQ
	bra.w		.STE_EntityOccludeVisibleXM_NONE_setup	; EMXSPRUR
	bra.w		.STE_EntityOccludeVisibleXM_EMSPR_setup	; EMHSPR
	bra.w		.STE_EntityOccludeVisibleXM_NONE_setup	; SLAB
	bra.w		.STE_EntityOccludeVisibleXM_NONE_setup	; CUSTOM
	bra.w		.STE_EntityOccludeVisibleXM_NONE_setup	; RFILL


*=======================================================*
*	dedicated iterator for [drawtype]=NONE
*-------------------------------------------------------*
.STE_EntityOccludeVisibleXM_NONE_next:
*-------------------------------------------------------*
	move.w		ent_drawtype(a0),d0
	cmp.w		.sp_drawtype(sp),d0
	bne		.STE_EntityOccludeVisibleXM_evaluate
*-------------------------------------------------------*
.STE_EntityOccludeVisibleXM_NONE_setup:
.STE_EntityOccludeVisibleXM_NONE_go:
*-------------------------------------------------------*
	; nothing todo here - skip!
*-------------------------------------------------------*
.STE_EntityOccludeVisibleXM_NONE_done:
*-------------------------------------------------------*
	move.l		ent_pnext_x(a0),a0		; next visible entity
	move.w		ent_frx(a0),d1
	cmp.w		.sp_pxymax_(sp),d1
	blt.s		.STE_EntityOccludeVisibleXM_NONE_next
	bra		.STE_EntityOccludeVisibleXM_complete
	
		
*=======================================================*
.STE_EntityOccludeVisibleXM_complete:
*-------------------------------------------------------*
