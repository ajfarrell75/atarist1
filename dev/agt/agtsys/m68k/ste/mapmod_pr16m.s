...
*------------------------------------------------------------------------------*
; offs,dst=a4,tmp=d0,dstpgh=d6,tdst=a3,tdstv=a6,tsrc=a5,msrc=a2
; busy: d4,d5
; free: usp,a1,d1

.macro	tile_pr16m	offs,dst,tmp,dstpgh,tdst,tdstv,tsrc,msrc

	lea		\offs*8(\dst),\tdst
	lea		\offs*8(\dst,\dstpgh.l),\tdstv
...
*------------------------------------------------------------------------------*
_do_dirtyrect_pr16m:	.globl	_do_dirtyrect_pr16m
*------------------------------------------------------------------------------*
...
*------------------------------------------------------------------------------*
	...
	; todo: with large guardband (e.g. 128) can actually be 21+8 or 29 bits!
...
.start4:
	...
	move.w		_dm_maskstride,.smc4m+2
	move.w		d2,.smc4s+2
	move.l		_dm_plmap,d0
	sub.l		a0,d0
	move.l		d0,.smc4t+2		
	move.l		_dm_ptiles,d2
	move.l		_dm_pltiles,d7
...
.ylp4:
	...
	; access transparent map layer as offset from base layer
...
.smc4m:	lea		1234(a1),a1
.smc4s:	lea		1234(a0),a0
	dbra		d4,.ylp4
	bra		.done
...
.smc3m:	lea		1234(a1),a1
.smc3s:	lea		1234(a0),a0
	dbra		d4,.ylp3
	bra		.done
...
.smc2m:	lea		1234(a1),a1
.smc2s:	lea		1234(a0),a0
	dbra		d4,.ylp2
	bra		.done
...
.smc1m:	lea		1234(a1),a1
.smc1s:	lea		1234(a0),a0
	dbra		d4,.ylp1
	bra		.done

*------------------------------------------------------------------------------*

;offs,dst=a4,tmp=d0,dstpgh=d6,tdst=a3,tdstv=a6,tsrc=a5,msrc=a2,mapsrc=a0,mmapsrc=a1,tilesrc=d2,mtilesrc=d7

.macro	jtile_pr16m	offs,dst,tmp,dstpgh,tdst,tdstv,tsrc,msrc,mapsrc,mmapsrc,tilesrc,mtilesrc
	...
	move.l		\mtilesrc,\msrc		; masked tile library
	add.l		\offs*4(\mmapsrc),\msrc	; masked map index

	; plot tile to relevant vpage(s)

	tile_pr16m	\offs,\dst,\tmp,\dstpgh,\tdst,\tdstv,\tsrc,\msrc
...
.do_block_1111:
	jtile_pr16m	-4,a4,d0,d6,a3,a6,a5,a2,a0,a1,d2,d7
.do_block_0111:
	jtile_pr16m	-3,a4,d0,d6,a3,a6,a5,a2,a0,a1,d2,d7
.do_block_0011:
	jtile_pr16m	-2,a4,d0,d6,a3,a6,a5,a2,a0,a1,d2,d7
.do_block_0001:
	jtile_pr16m	-1,a4,d0,d6,a3,a6,a5,a2,a0,a1,d2,d7
	lea		-4*4(a0),a0
	lea		-4*4(a1),a1
	lea		-4*8(a4),a4	
	rts
