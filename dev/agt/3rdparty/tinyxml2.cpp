...
NOTICE: 
 THIS SOURCE HAS BEEN MODIFIED FOR USE WITH ATARI/M68K GCC & AGT
 CHANGES HAVE BEEN MARKED "dml:"
 dml / 2017

*/

#include "tinyxml2.h"
//#define TIXML_USE_FLOAT
...
extern "C" int vsnprintf (char *str, size_t count, const char *fmt, va_list arg);
...
{
    if ( !p || !(*p) ) {
		//printf("ParseName:in:0\n");
        return 0;
    }
    if ( !XMLUtil::IsNameStartChar( *p ) ) {
		//printf("ParseName:IsNameStartChar:0\n");
        return 0;
    }
    ...
    if ( *(p+1) == '#' && *(p+2) ) {
        ...
        if ( *(p+2) == 'x' ) {
            ...
            while ( *q != 'x' ) {
                ...
                TIXMLASSERT( digit == 0 || mult <= INT_MAX / digit );
                const int digitScaled = mult * digit;
                TIXMLASSERT( ucs <= LONG_MAX - digitScaled );
                ucs += digitScaled;
                TIXMLASSERT( mult <= INT_MAX / 16 );
                mult *= 16;
                --q;
            }
        }
            ...
            while ( *q != '#' ) {
                if ( *q >= '0' && *q <= '9' ) {
                    const unsigned short digit = *q - '0';
...
char* XMLDocument::Identify( char* p, XMLNode** node )
{
    ...
    if( !*p ) {
        *node = 0;
        TIXMLASSERT( p );
		//agtprintf("Identify:SkipWhiteSpace:fail\n");
        return p;
    }

	//char buf[256];
	//snprintf(buf, 8, "%s", p);
	//agtprintf("Identify:[%s]\n", buf);
    ...
    if ( XMLUtil::StringEqual( p, xmlHeader, xmlHeaderLen ) ) {
		//agtprintf("Identify:xmlHeader\n");
        returnNode = CreateUnlinkedNode<XMLDeclaration>( _commentPool );
        returnNode->_parseLineNum = _parseCurLineNum;
        p += xmlHeaderLen;
    }
    else if ( XMLUtil::StringEqual( p, commentHeader, commentHeaderLen ) ) {
		//agtprintf("Identify:commentHeader\n");
        returnNode = CreateUnlinkedNode<XMLComment>( _commentPool );
        returnNode->_parseLineNum = _parseCurLineNum;
        p += commentHeaderLen;
    }
    else if ( XMLUtil::StringEqual( p, cdataHeader, cdataHeaderLen ) ) {
		//agtprintf("Identify:dataHeader\n");
		...
		//agtprintf("Identify:dtdHeader\n");
        returnNode = CreateUnlinkedNode<XMLUnknown>( _commentPool );
        returnNode->_parseLineNum = _parseCurLineNum;
        p += dtdHeaderLen;
    }
    else if ( XMLUtil::StringEqual( p, elementHeader, elementHeaderLen ) ) {
		//agtprintf("Identify:elementHeader\n");
...
char* XMLElement::ParseDeep( char* p, StrPair* strPair, int* curLineNumPtr )
{
    ...
    if ( _value.Empty() ) {
		//printf("ParseText:ParseName:valueempty:0\n");
        return 0;
    }

    p = ParseAttributes( p, curLineNumPtr );
	//if (!p)
	//	printf("ParseDeep:ParseAttributes:0\n");
    if ( !p || !*p || _closingType ) {
        return p;
    }
...
XMLError XMLDocument::LoadFile( const char* filename )
{
    ...
    if ( !fp ) {
		...
		//agtprintf("XML_ERROR_FILE_NOT_FOUND 0\n");
        return _errorID;
    }
    LoadFile( fp );
    fclose( fp );
...
XMLError XMLDocument::LoadFile( FILE* fp )
{
    ...
    if ( fgetc( fp ) == EOF && ferror( fp ) != 0 ) {
		...
		//agtprintf("XML_ERROR_FILE_READ_ERROR 1\n");
        return _errorID;
    }
    ...
    if ( filelength == -1L ) {
        SetError( XML_ERROR_FILE_READ_ERROR, 0, 0, 0 );
		//agtprintf("XML_ERROR_FILE_READ_ERROR 2\n");
        return _errorID;
    }
    ...
    if ( !LongFitsIntoSizeTMinusOne<>::Fits( filelength ) ) {
        ...
        SetError( XML_ERROR_FILE_READ_ERROR, 0, 0, 0 );
		//agtprintf("XML_ERROR_FILE_READ_ERROR 3\n");
        return _errorID;
    }

    if ( filelength == 0 ) {
        SetError( XML_ERROR_EMPTY_DOCUMENT, 0, 0, 0 );
		//agtprintf("XML_ERROR_EMPTY_DOCUMENT 4\n");
        return _errorID;
    }
    ...
    if ( read != size ) {
        SetError( XML_ERROR_FILE_READ_ERROR, 0, 0, 0 );
		//agtprintf("XML_ERROR_FILE_READ_ERROR 5\n");
        return _errorID;
    }
...
void XMLDocument::Parse()
{
    ...
    if ( !*p ) {
        SetError( XML_ERROR_EMPTY_DOCUMENT, 0, 0, 0 );
		//agtprintf("XML_ERROR_EMPTY_DOCUMENT\n");
        return;
    }
