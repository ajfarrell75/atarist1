...
<
	...
	/*guardy=*/0								// removes guardband from screen split = more efficient
...
static drawcontext_t drawcontext1;
static drawcontext_t drawcontext2;
...
static void update_viewport(entity_t *_pviewport, arena_t &_arena, drawcontext_t &_drawcontext, int _buffer_index, int _field_index/*, int _viewbind*/)
{
	EntitySelectViewport(_pviewport);

	// select the next workbuffer (..of two - we're using double buffering here)
	_arena.select_buffer((_buffer_index & (4-1)));

	// update the playfield coordinates for this draw pass
	_arena.setpos(_pviewport->rx+c_viewport_xmargin, _pviewport->ry+c_viewport_ymargin /*g_viewport_worldx, g_viewport_worldy*/);
	...
	_arena.get_context(_drawcontext, working_field, VIEWPORT_XSIZE, _arena.display_height_);
...
int AGT_EntryPoint()
{
	...
	// ---------------------------------------------------------------------------------------------------------------------
	{
		...
		arena_t arena_view_1(shift, VIEWPORT_XSIZE, VIEWPORT_YSIZE);
		arena_t arena_view_2(shift, VIEWPORT_XSIZE, VIEWPORT_YSIZE);
...
//		myworld.setmap(&mymap0);
		...
		{
			// note: currently first valid line to schedule with Nickel is line #5 but this should be fixed in future
			{ NE_PortSelect,	0+5,					NickelPort0	},
			{ NE_PortSelect,	(VIEWPORT_YSIZE*1)+5,	NickelPort1	},
			{ NE_Stop,			(VIEWPORT_YSIZE*2)+5	}
		};
		...
		arena_view_1.init(g_viewport_worldx, g_viewport_worldy);
		arena_view_2.init(g_viewport_worldx, g_viewport_worldy);

		// provides quick access to tilemap rows
//		s_pmaplineindex = myworld.buffers_[0].fields[0].pmapcontext_->mul_map_x_;
		s_pmaplineindex = arena_view_1.buffers_[0].fields[0].pmapcontext_->mul_map_x_;
...
//		myworld.reset(g_viewport_worldx, g_viewport_worldy);
		arena_view_1.reset(g_viewport_worldx, g_viewport_worldy);
		arena_view_2.reset(g_viewport_worldx, g_viewport_worldy);
...
mainloop:
			...
			s_pe_viewport1->rx = g_viewport_worldx - c_viewport_xmargin;
			s_pe_viewport1->ry = g_viewport_worldy - c_viewport_ymargin;

			s_pe_viewport2->rx = ((g_map_fxlimit>>16) - g_viewport_worldx) - c_viewport_xmargin;
			s_pe_viewport2->ry = ((g_map_fylimit>>16) - g_viewport_worldy) - c_viewport_ymargin;
			...
			update_viewport(s_pe_viewport1, arena_view_1, drawcontext1, buffer_index, field_index);
			update_viewport(s_pe_viewport2, arena_view_2, drawcontext2, buffer_index, field_index);
			...
			static s16 ccc = 32; // static, so it's initialised only once on first use!
			if (ccc-- < 0)
			{
				ccc = 256;
...
/*
	...
	if ((spx > (320+32)) || (spy > (SCREEN_LINES+32)))
		return;
*/
	// allow tick if overlapping either viewport
	// otherwise, sleep to save CPU
	{
		u16 spx = self.rx - (s_pe_viewport1->rx-0);
		u16 spy = self.ry - (s_pe_viewport1->ry-0);
		if ((spx <= (VIEWPORT_XSIZE+(c_viewport_xmargin*2))) && (spy <= (VIEWPORT_YSIZE+(c_viewport_ymargin*2))))
		{
			// continue
		}
		else
		{
			u16 spx = self.rx - (s_pe_viewport2->rx-0);
			u16 spy = self.ry - (s_pe_viewport2->ry-0);
			if ((spx > (VIEWPORT_XSIZE+(c_viewport_xmargin*2))) || (spy > (VIEWPORT_YSIZE+(c_viewport_ymargin*2))))
				return;
		}
	}
