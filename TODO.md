...
| **The Cuddly Demos** (TCB) | 1ʳᵉ page OK ; **menu robot : image qui saute** (bascule verticale {0,−3,−6}/trame + briques en escalier ; oracle Hatari = mur pleine largeur STABLE). | **Diagnostic posé (2026-07-02, traces `video_border_h` + NEOST_GLUE_DIAG)** : Hatari met à jour PAR ÉCRITURE `hbl_pos`/`cycles_line` par ligne (ligne hi précoce → **224 cyc**, restaurée 512 par le lo) ET la fenêtre VERTICALE `V_DE` en live (haut 63→34 via paire 60/50 à cheval de ligne, bas 234→434) — NeoST : grille fixe 512 + V_DE par trame (V2 opt-in partiel « EMPIRE »). Le menu exerce tout (paires res hi/lo @4/12 = gauche+ligne 224 ; sync 60@376/50@384 = droite tenue ; res @444/456 = right-full enchaîné → LEFT_OFF ligne suivante). La démo étant beam-syncée, la géométrie fausse fait diverger SON automate. Chantier = port fidèle nCyclesPerLine/HBL_Pos + V_DE live (Video_Update_Glue_State chemins hbl_pos/cycles_line). Menu headless : `--fastfdc --keys-at 3000 " "`, ≈ trame 6000 ; oracle AVI + trace border capturés. |
