...
| **The Cuddly Demos** (TCB) | 1ʳᵉ page OK ; **menu robot : clignotement vertical** (~47 % trames : fenêtre oscille **34..310** ↔ **63..263** ; briques en escalier ; oracle Hatari = mur pleine largeur STABLE). | **Diagnostic raffiné (2026-07-02 fin de session, mesures)** : chaîne causale — (1) ~47 % trames ratent retrait bordure haute ; (2) trames ratées : 1ʳᵉ paire 60/50 à **L34** au lieu de **L33** ; (3) poll son **`pc=f264`** lit **`$8209`** (verrouillée → avance dès L34 ; ratée → gelé, **Δ=−160/ligne**) → **boucle bistable** vs attracteur Hatari « toujours verrouillé » (écart initial **±160 octets**). **Éliminé** : canal `HBL_Pos`/`nCyclesPerLine` (`c680e1a`/`10c72b8`, `NEOST_LINELEN`) ; sortie fenêtre retrait haut ; masques Glue par ligne. **Reste** : fenêtre verticale **3 lignes** (VDE_On 34 vs 35, comptage bordure haute, ou `Video_RestartVideoCounter`). **Prochaine sonde** : oracle Hatari `--trace video_addr` → `$8209` **L35** vs NeoST. Menu headless : `--fastfdc --keys-at 3000 " "`, ≈ trame 6000. → `docs/MOIRA_WINUAE_CONVERGENCE.md`. |
