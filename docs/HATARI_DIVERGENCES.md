...
| Blitter | `Blitter.cpp` | `blitter.c` | très élevée sur les **données** (byte-identique à l'oracle, mesuré 2026-08-26) ; base de temps unifiée (BL3 ✅) et dispatch par accès (BL4 ✅) | **1 (BL5, dérive ~86 cyc/blit non-hog)** / 2 (CPU parallèle, BL-R) |
...
| BL5 | Blitter / MFP | **Dérive CUMULATIVE de la datation d'un blit non-hog, ~86 cyc par blit.** Découverte le 2026-08-26 par le nouvel étalon `blitter_timer` (chantier A2), **dès sa première exécution**. Le chemin de **DONNÉES est byte-identique** à Hatari (0 px sur les 8 lignes de destination) ; l'écart est **entièrement de datation** : 99 lignes TADR sur 100, avec une dérive qui s'ACCUMULE de +1 à **+43 tics** de prescaler sur 100 blits (1 tic = 200 cyc ⇒ ≈ **86 cyc/blit**). L'oracle est **stable** (3 runs Hatari = 0 px entre eux), ce n'est donc pas un artefact de boot. **BL3/BL4 ont RAPPROCHÉ** NeoST de l'oracle sans fermer l'écart : **463 px avant → 397 px après**. ⚠ **DEUX hypothèses ont été formulées puis RÉFUTÉES le jour même — ne pas les rouvrir.** (1) « NeoST facture l'arbitration une fois par blit, Hatari une fois par tranche » : **FAUX**. `Blitter_Start` (`blitter.c:871`) est RÉ-APPELÉ à chaque tranche via `INTERRUPT_BLITTER`, et fait `Blitter_BusArbitration(BUS_MODE_BLITTER)` en entrée + `(BUS_MODE_CPU)` en sortie — soit exactement ce que fait `Blitter::onSlice` depuis BL4. (2) « l'oracle tourne le chemin non-CE, donc le forfait plat de 256 cyc (`blitter.c:943`) au lieu du comptage d'accès CPU » : **FAUX AUSSI**. Mesuré : le défaut d'Hatari sur ce banc EST le cycle-exact — `--cpu-exact on --compatible on` donne le même résultat que sans option (397 px), et ce n'est qu'en forçant les DEUX à `off` que l'image change (328 px). `BLITTER_RUN_CE` est donc VRAI dans l'oracle : les deux émulateurs sont sur le même modèle. **La cause reste OUVERTE.** ⚠ Toute reprise doit ÉPINGLER `--cpu-exact` et `--compatible` : ils déplacent le résultat de 69 px, et `tools/hatari_oracle.sh` n'en passe aucun (il prend donc les défauts d'Hatari, qu'il faut vérifier plutôt que supposer). **LOCALISÉE le 2026-08-26** par l'étalon jumeau `blitter_hog` (même programme, `ctrl=$C0`) : sur le chemin **HOG**, NeoST est **CONFORME à l'oracle, 0 px** (zone TADR comme zone données, `ref_kind: oracle`). La divergence est donc **spécifique au NON-HOG**, c'est-à-dire à la **fenêtre CPU entre deux tranches** — `Blitter::noteCpuBusAccess` (64 accès bus CPU + `kPreStartCycles`) face à `BLITTER_PHASE_COUNT_CPU_BUS` / `Blitter_HOG_CPU_IgnoreMaxCpuCycles` chez Hatari. La facturation du blit lui-même est hors de cause : elle est identique dans les deux modes, et le mode HOG est propre. | moyenne (cycle-exactness) | `Blitter.cpp:billCycles` / `onSlice` / `start` | `Blitter_BusArbitration` `blitter.c:395-421`, `Blitter_FlushCycles` `blitter.c:356-374` |
...
| D3 | FDC/DMA | Flush FIFO↔RAM **ne stalle pas le CPU** (32 cyc manquants). **CONFIRMÉ ET CHIFFRÉ (2026-08-25, trois reproductions indépendantes)** — cycles par flush de 16 o : Hatari **4127**, NeoST défaut **4173**, NeoST sans latence de dispatch **4100**, modèle sans stall **4096**. NeoST est donc **déjà +45 cyc trop lent** par flush. ⚠ **NE PAS APPLIQUER SEUL** : ajouter les 32 cyc sans l'ancre anti-dérive (candidat n°2, `HATARI_MAPPING.md`) porterait l'erreur de +45 à **+77** — *D3 seul est une RÉGRESSION*. Couplé, on retombe à ≈ **+5**. ⚠ Sous `--fastfdc` — le mode réellement employé par les étalons et le tier fast — l'erreur actuelle est de **+14,1 %** de temps de chargement (492,7 vs 431,9 cyc/flush), pas 1,1 %. Coût de validation : `--tier full` **obligatoire**, avec régénération possible des références `snapshot` prises à trame fixe (`nocooper_greetings` f29200, `overscan_top` f380), que le déplacement de la durée de boot peut faire bouger. | moyenne *(confirmée)* | `Fdc.cpp` (`fifoPush`/`fifoPull`) | `FDC_DMA_FIFO_Push/Pull` `fdc.c`, `M68000_AddCycles_CE` |
...
- **[V3 — basse] ◐ partiellement résolu** — `RestartVideoCounter` **PORTÉ** (2026-07-02,
