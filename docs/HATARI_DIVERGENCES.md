...
| D3 | FDC/DMA | Flush FIFO↔RAM **ne stalle pas le CPU** (32 cyc manquants). **CONFIRMÉ ET CHIFFRÉ (2026-08-25, trois reproductions indépendantes)** — cycles par flush de 16 o : Hatari **4127**, NeoST défaut **4173**, NeoST sans latence de dispatch **4100**, modèle sans stall **4096**. NeoST est donc **déjà +45 cyc trop lent** par flush. ⚠ **NE PAS APPLIQUER SEUL** : ajouter les 32 cyc sans l'ancre anti-dérive (candidat n°2, `HATARI_MAPPING.md`) porterait l'erreur de +45 à **+77** — *D3 seul est une RÉGRESSION*. Couplé, on retombe à ≈ **+5**. ⚠ Sous `--fastfdc` — le mode réellement employé par les étalons et le tier fast — l'erreur actuelle est de **+14,1 %** de temps de chargement (492,7 vs 431,9 cyc/flush), pas 1,1 %. Coût de validation : `--tier full` **obligatoire**, avec régénération possible des références `snapshot` prises à trame fixe (`nocooper_greetings` f29200, `overscan_top` f380), que le déplacement de la durée de boot peut faire bouger. ⚠ **TENTÉ DEUX FOIS LE 2026-08-26, ET REVERTÉ — mais les chiffres de la première tentative étaient FAUX, cf. ci-dessous.** Les deux volets ont été implémentés comme `HATARI_MAPPING.md` les décrit : le stall de 32 cyc aux deux flushs (facturé avec l'**invariant BL3** — `addBusWaitCycles` PLUS `Scheduler::addStolenCycles`, car `fifoPush`/`fifoPull` tournent dans `Fdc::onFdcEvent`, donc HORS quantum) et l'ancrage anti-dérive sur l'échéance servie (`Scheduler::firingDue()` ≙ `-PendingCyclesOver`). 🛑 **CORRECTION D'UNE MESURE PUBLIÉE.** La première passe concluait « l'ancrage DOUBLE le débit DMA ». **C'est FAUX** : la sonde était logée DANS `billDmaCycles`, c'est-à-dire dans le correctif lui-même, donc elle ne tournait pas côté « avant » — zéro ligne, que j'ai lu comme « aucun flush régulier ». Un banc de comparaison doit être ACTIF DES DEUX CÔTÉS de ce qu'il compare. La sonde est désormais appelée depuis les SITES DE FLUSH (`Fdc::noteFifoFlush`), donc active dans toutes les variantes. **Mesures refaites** (Blood Money, régime de rafale) : sans couple **4203** cyc/flush, stall seul **4235**, **ancrage + stall 4096**, cible Hatari **4127**. Le couple RAPPROCHE donc de l'oracle (−31 contre +76) — l'inverse de ce qui avait été écrit. Mécanisme chiffré par une seconde sonde (`NEOST_FDC_LATE_DIAG=1`) : le dispatch FDC arrive avec **6,7 cyc de retard moyen** (max 126), soit ~107 cyc accumulés par cycle de 16 octets — exactement l'écart 4203 → 4096 que l'ancrage supprime. **ET POURTANT IL FAUT REVERTER.** Avec le couple, **10 étalons pixel sur 12 passent** (`cuddly_demos` et `spectrum512_diapo` compris, qui bootent pourtant un disque), mais `nocooper` et `nocooper_greetings` tombent. Ce n'est PAS un décalage de trame dû au chargement plus rapide : un balayage oracle élargi à **±400 trames (801 images)** donne une meilleure correspondance à **~65 000 px** — une image franchement différente, alors que SANS le couple `nocooper` est à **0 px** de l'oracle. Le couple casse donc le rendu d'une démo beam-synchronisée tout en améliorant le débit : quelque chose compense dans le modèle actuel, et le couple le déséquilibre. **Piste pour la reprise** : le stall n'atterrit pas dans la cadence propre du FDC (4096 = 16 × 256 = le modèle SANS stall), l'ancrage l'absorbe. Une tentative de le réinjecter sur le réarmement donne 4235, c'est-à-dire l'ancrage neutralisé — la bonne formulation reste à trouver. Sondes conservées : `NEOST_FDC_FLUSH_DIAG` et `NEOST_FDC_LATE_DIAG`. | moyenne *(confirmée, couple insuffisant)* | `Fdc.cpp` (`fifoPush`/`fifoPull`) | `FDC_DMA_FIFO_Push/Pull` `fdc.c`, `M68000_AddCycles_CE` |
