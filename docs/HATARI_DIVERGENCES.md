...
| D3 | FDC/DMA | Flush FIFO↔RAM **ne stalle pas le CPU** (32 cyc manquants). **CONFIRMÉ ET CHIFFRÉ (2026-08-25, trois reproductions indépendantes)** — cycles par flush de 16 o : Hatari **4127**, NeoST défaut **4173**, NeoST sans latence de dispatch **4100**, modèle sans stall **4096**. NeoST est donc **déjà +45 cyc trop lent** par flush. ⚠ **NE PAS APPLIQUER SEUL** : ajouter les 32 cyc sans l'ancre anti-dérive (candidat n°2, `HATARI_MAPPING.md`) porterait l'erreur de +45 à **+77** — *D3 seul est une RÉGRESSION*. Couplé, on retombe à ≈ **+5**. ⚠ Sous `--fastfdc` — le mode réellement employé par les étalons et le tier fast — l'erreur actuelle est de **+14,1 %** de temps de chargement (492,7 vs 431,9 cyc/flush), pas 1,1 %. Coût de validation : `--tier full` **obligatoire**, avec régénération possible des références `snapshot` prises à trame fixe (`nocooper_greetings` f29200, `overscan_top` f380), que le déplacement de la durée de boot peut faire bouger. ⚠ **TENTÉ LE 2026-08-26, ET REVERTÉ — la prescription du couple est INCOMPLÈTE.** Les deux volets ont été implémentés exactement comme `HATARI_MAPPING.md` les décrit : le stall de 32 cyc aux deux flushs (`4 * FDC_DMA_FIFO_SIZE / 2`, facturé avec l'**invariant BL3** — `addBusWaitCycles` PLUS `Scheduler::addStolenCycles`, car `fifoPush`/`fifoPull` tournent dans `Fdc::onFdcEvent`, donc HORS quantum), et l'ancrage anti-dérive sur l'échéance SERVIE (`Scheduler::firingDue()` ≙ `-PendingCyclesOver`). **Mesuré** par une sonde ajoutée pour l'occasion et CONSERVÉE (`NEOST_FDC_FLUSH_DIAG=1`, moyenne restreinte au régime de **rafale** — intervalle < 8000 cyc ; une moyenne sur TOUS les flushs est polluée par les seeks et donne ~19000, sans rapport avec un débit de rafale) : **avant**, ZÉRO intervalle sous 8000 cyc sur tout un chargement de Blood Money ; **après**, 15000 intervalles réguliers à **4096,1 cyc/flush**. L'ancrage **DOUBLE le débit DMA** — ce n'est pas le raffinement de ~5 cyc annoncé. ⚠ Et 4096 = 16 octets × 256 cyc, c'est-à-dire EXACTEMENT le modèle SANS stall : l'ancrage **absorbe** les 32 cycles pour la cadence propre du FDC, ce qui ne peut pas être ce que fait Hatari puisqu'il mesure 4127. C'est la piste la plus sérieuse pour la reprise. **Verdict** : `nocooper` (référence **ORACLE**) et `nocooper_greetings` tombent, et un balayage oracle de **±90 trames (181 images)** ne trouve **aucune** trame correspondante — NeoST s'ÉLOIGNE de l'oracle. Reverté ; seule la sonde est conservée. | moyenne *(confirmée, couple insuffisant)* | `Fdc.cpp` (`fifoPush`/`fifoPull`) | `FDC_DMA_FIFO_Push/Pull` `fdc.c`, `M68000_AddCycles_CE` |
