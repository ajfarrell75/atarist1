...
| **Closure** | Sync | Démo moderne à **rasters fins** : la palette est réécrite en cours de ligne sur des écrans denses (64→129 couleurs, puis **153 couleurs** sur la « photo fée » vers la trame 10500). Se joue en **ST + 1 Mo** ; le chemin **STE n'est PAS porté** (Hatari lui-même : « FIXME: should be measured on real STE », `video.c:3966-3970`) — ne pas la lancer sous une ROM STE-only. | **A40 : décalage PERMANENT de la fenêtre gauche.** Posée en étalon le 2026-08-30, elle a immédiatement montré un écart oracle de 64,15 % dont la cause est GÉOMÉTRIQUE, pas chromatique : palettes identiques, et **+4 px ST** ramène l'écart à 3,20 % (±1 px → ~29 %, donc exactement 4 ; revérifié en 832×552). C'est `glue::LEFT_OFF`. Contrairement à `overscan_top` (5 lignes de transition), l'écart est **uniforme sur les 276 lignes** : A40 n'est donc pas une singularité du retrait de bordure haute. | Étalon `closure` (écran 153 couleurs, `--frames 10500`, ST 1 Mo, `etos192fr`) — **`ref_kind: snapshot` à dessein** : poser l'oracle aurait FIGÉ l'écart A40 au lieu de le signaler. Le disque est dans le dépôt (`disks/etalons/closure.msa`), pas de fetch. ~20 s, 2ᵉ mur du palier pixel. Journal d'enquête : `docs/CLOSURE_CHANTIER.md` |
