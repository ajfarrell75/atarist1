#!/usr/bin/env bash
# setup_neost.sh — installe TOUTES les dépendances de NeoST puis compile.
#
# Remplace l'ancien setup.sh (qui référençait encore le cœur Musashi retiré).
# Étapes :
#   1) paquets système : GLFW3 + OpenGL + CMake + toolchain C++17  (brew/apt/pacman)
#   2) sous-modules     : extern/imgui, extern/miniaudio, extern/moira
...
#
# Usage :
#   ./setup_neost.sh            # build Release complet
#   ./setup_neost.sh --debug    # build Debug
#   ./setup_neost.sh --no-deps  # saute l'installation des paquets système
set -euo pipefail
...
say "Terminé. Lance l'émulateur avec : ./run_neost.sh"
