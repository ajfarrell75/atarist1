...
for rom in etos192us.img etos192fr.img etos256us.img etos256fr.img tos162uk.img; do
...
# Garde-fou : seules EmuTOS + tos162uk.img (profil STE) sont autorisées.
# (pas de -printf : find BSD de macOS ne le connaît pas)
STRAY=$(find "$DEST/roms" -maxdepth 1 -name '*.img' ! -name 'etos*' ! -name 'tos162uk.img')
if [ -n "$STRAY" ]; then
    echo "ERREUR : ROM non autorisée dans le paquet : $STRAY" >&2
    exit 1
fi
echo "OK : données paquet copiées dans $DEST (EmuTOS + tos162uk + drivesound + diskA.st)"
