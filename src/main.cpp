...
// Mode HARNAIS (--run-frames, A9a 2026-08-27) : un run de test ne doit laisser AUCUNE
// trace dans l'état utilisateur. Le gel est CENTRAL (ici, comme celui du kiosk) parce
// que saveConfig a une demi-douzaine d'appelants — démarrage (rom=), résolution MIDI à
// la première trame (le site qui, lancé d'un contexte sandboxé où CoreMIDI échoue,
// remettait silencieusement midi_out_port=0 — incident déjà payé), montages, sortie.
// Sans ce gel, brancher le boot GUI dans run_all.py réécrivait le neost.cfg du
// développeur à chaque palier (constaté : rom= et rtc= écrasés au premier essai).
...
static void saveConfig(const std::string& exeDir, Config& c, Machine* machine = nullptr, bool force = false) {
    if (g_harnessRun) return;                             // harnais : zéro écriture, quel que soit l'appelant
...
int main(int argc, char** argv) {
    ...
    // --run-frames / --shot (cf. le parseur ci-dessous). Le drapeau harnais associé
    // (g_harnessRun, gel d'écriture de neost.cfg) vit au niveau fichier, près de
    // saveConfig qu'il verrouille.
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (a == "--run-frames" && i + 1 < argc) { g_runFrames = std::atol(argv[++i]); g_harnessRun = true; }
    ...
    if (g_harnessRun) ImGui::GetIO().IniFilename = nullptr;
#ifdef IMGUI_HAS_DOCK
    ...
    // (En mode harnais, saveConfig est gelé centralement — cf. g_harnessRun.)
    cfg.disk = machine.fdc.mountedPath();
    cfg.cart = machine.bus.mountedCartPath();
    cfg.mono = !machine.mfp.colorMonitor();
    ...
    if (!g_harnessRun && ImGui::GetIO().IniFilename)
        ImGui::SaveIniSettingsToDisk(ImGui::GetIO().IniFilename);
